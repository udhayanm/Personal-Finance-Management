...
function ExpenseList({ type, yearActive, monthActive, btn=true }) {
    const initVals = { id: '', name: '', price: '', date: '' };
    
    const [data, setData] = useState([]);
    const [totExpense, setTotExpense] = useState(null);

    const [editing, setEditing] = useState(false);
    const [editData, setEditData] = useState(initVals);
    ...
    const handleUpdate = e => {
        ...
        if (editData.date.slice(0, 7) !== existingExpense.date.slice(0, 7)) {
            ...
            const oldQueryYear = yearActive;
            const oldExpensePath = `/users/${auth.currentUser.uid}/${type}/${oldQueryYear}/${oldQueryMonth}/${expenseId}`;
            const newExpensePath = `/users/${auth.currentUser.uid}/${type}/${editData.date.slice(0, 4)}/${editData.date.slice(5, 7)}/${editData.id}`;
    ...
    const handleDelete = e => {
        ...
        const expenseMonth = parseInt(e.target.attributes['data-value'].nodeValue);
        const queryMonth = monthActive === 'all' ? expenseMonth : monthActive;
        const expensePath = `/users/${auth.currentUser.uid}/${type}/${yearActive}/${queryMonth}/${expenseId}`;
